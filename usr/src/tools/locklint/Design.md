...
formal-relative and absolute locks are candidates only when a direct or
transitive callee acquisition reaches that source-relative lock; this excludes
assertion-only state.  Candidate effects are propagated to a fixed point over
resolved call edges, rebasing formal-relative accesses at each call.  Outcomes
from all root-reachable ordinary contexts are combined by source-relative
identity, and a matching declared acquisition suppresses the generic report
because its contract is validated separately.
