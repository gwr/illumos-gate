...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/lock-declared.c"
...
run_root="$RESULTS_ROOT/lock-declared-$timestamp-$$"
...
run_variant()
{
	...
	print "Compiling $variant declared-lock fixture"
	(
		cd "$build_dir" || exit 1
		"$CC" -Zll -DLOCK_DECLARED_VARIANT="$variant" "$SOURCE"
	...
	OSLL_LL_FILE="$build_dir/lock-declared.ll"
	...
	print "Running OSLL for variant $variant"
...
run_variant 1 "call_declared_acquire_unheld call_declared_acquire_held \
call_declared_acquire_held_contract declared_acquire_missing \
declared_acquire_conditional undeclared_acquire \
declared_acquire_duplicate" || exit $?
run_variant 2 "call_declared_release_held call_declared_release_unheld \
call_declared_release_unheld_contract declared_release_missing \
declared_release_conditional undeclared_release" || exit $?
run_variant 3 "assert_after_acquire assert_held_conditionally \
call_require_held_locked call_require_held_unlocked \
call_require_not_held_unlocked call_require_not_held_locked \
call_require_held_wrapper_unlocked" || exit $?
run_variant 4 "declared_acquire_no_return declared_release_no_return" ||
    exit $?
