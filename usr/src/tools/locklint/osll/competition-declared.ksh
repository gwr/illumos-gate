...
run_session()
{
	...
	0|5)
		...
		run_command vars-after "$LOCK_LINT" vars
		exit 0
		;;
	*)
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/competition-declared.c"
...
run_root="$RESULTS_ROOT/competition-declared-$timestamp-$$"
...
run_variant()
{
	...
	print "Compiling $variant declared-competition fixture"
	(
		cd "$build_dir" || exit 1
		"$CC" -Zll -DCOMPETITION_DECLARED_VARIANT="$variant" \
		    "$SOURCE"
	...
	OSLL_LL_FILE="$build_dir/competition-declared.ll"
...
run_variant 1 "competition_balanced competition_nested_balanced \
competition_nested_enter_effect competition_nested_leave_effect \
competition_opposite_branches competition_unmatched_leave" || exit $?
run_variant 2 "declared_no_competition_valid declared_no_competition_missing \
declared_no_competition_conditional undeclared_no_competition \
declared_competition_valid undeclared_competition" || exit $?
run_variant 3 "call_competition_leave_contract \
call_competition_leave_no_contract call_competition_wrapper_contract \
call_competition_wrapper_no_contract" || exit $?
