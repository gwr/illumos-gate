...
#
# Run focused rw_tryenter result-sensitive cases under Solaris LockLint.
#
...
run_session()
{
	...
	for root in \
	    tryenter_reader_truth \
	    tryenter_reader_write \
	    tryenter_starvewriter_truth \
	    tryenter_writer_truth \
	    tryenter_reader_failure \
	    tryenter_writer_failure \
	    tryenter_saved_result \
	    tryenter_ignored_result \
	    call_balanced_tryenter \
	    tryenter_order_on_success \
	    tryenter_reader_while_reader \
	    tryenter_writer_while_reader \
	    tryenter_reader_while_writer \
	    tryenter_writer_while_writer
	do
		run_command "declare-root-$root" "$LOCK_LINT" declare root \
		    "rwlock-tryenter.c:$root" || exit $?
	done
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/rwlock-tryenter.c"
...
OSLL_LL_FILE="$build_dir/rwlock-tryenter.ll"
...
case "$analyze_status" in
0)
	print "OSLL rwlock-tryenter analysis completed without findings."
	;;
5)
	print "OSLL rwlock-tryenter analysis completed with findings."
	;;
*)
	print "OSLL rwlock-tryenter analysis returned status $analyze_status."
	;;
esac
