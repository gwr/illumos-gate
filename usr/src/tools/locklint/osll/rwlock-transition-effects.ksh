...
#
# Run declared rwlock upgrade and downgrade side-effect cases under Solaris
# LockLint.  Variants isolate the two annotations and their invalid contracts.
#
...
run_session()
{
	...
	do
		run_command "declare-root-$root" "$LOCK_LINT" declare root \
		    "rwlock-transition-effects.c:$root" || exit $?
	done
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/rwlock-transition-effects.c"
...
run_root="$RESULTS_ROOT/rwlock-transition-effects-$timestamp-$$"
...
run_variant()
{
	...
	print "Compiling variant $variant rwlock transition effects"
	(
		cd "$build_dir" || exit 1
		"$CC" -Zll -D_KERNEL \
		    -DRWLOCK_TRANSITION_EFFECT_VARIANT="$variant" "$SOURCE"
	...
	OSLL_LL_FILE="$build_dir/rwlock-transition-effects.ll"
...
run_variant 1 "call_upgrade_reader call_upgrade_writer \
call_upgrade_unheld declared_upgrade_ignored declared_upgrade_missing \
declared_upgrade_conditional" || exit $?
run_variant 2 "call_downgrade_writer call_downgrade_reader \
call_downgrade_unheld declared_downgrade_missing \
declared_downgrade_conditional" || exit $?
