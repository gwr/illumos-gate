...
#
# Run the focused acquisition-prefix alias cases under Solaris LockLint.
#
...
run_session()
{
	...
	do
		run_command "declare-root-$root" "$LOCK_LINT" declare root \
		    "acquisition-prefix-alias.c:$root" || exit $?
	done
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/acquisition-prefix-alias.c"
...
run_root="$RESULTS_ROOT/acquisition-prefix-alias-$timestamp-$$"
...
run_variant()
{
	...
	print "Compiling acquisition-prefix-alias variant $variant"
	(
		cd "$build_dir" || exit 1
		"$CC" -Zll -DACQUISITION_PREFIX_ALIAS_VARIANT="$variant" \
		    "$SOURCE"
	...
	OSLL_LL_FILE="$build_dir/acquisition-prefix-alias.ll"
	...
	print "Running OSLL acquisition-prefix-alias variant $variant"
...
run_variant 1 "same_release_before_target \
