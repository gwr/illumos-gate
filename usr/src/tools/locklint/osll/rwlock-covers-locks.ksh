...
#
# Run focused RWLOCK_COVERS_LOCKS protection, ownership, identity, and wrapper
# cases under Solaris LockLint.
#
...
run_session()
{
	...
	for root in \
	    access_with_cover_writer \
	    access_with_cover_reader \
	    acquire_with_cover_reader \
	    acquire_with_cover_writer \
	    acquire_without_cover \
	    release_cover_while_covered \
	    acquire_other_object \
	    call_wrapper_with_cover \
	    call_wrapper_without_cover \
	    call_release_wrapper_while_covered \
	    call_access_wrapper_with_writer \
	    call_access_wrapper_with_reader \
	    access_absolute_cover_writer \
	    acquire_absolute_without_cover
	do
		run_command "declare-root-$root" "$LOCK_LINT" declare root \
		    "rwlock-covers-locks.c:$root" || exit $?
	done
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/rwlock-covers-locks.c"
...
OSLL_RESULT_DIR="$RESULTS_ROOT/rwlock-covers-locks-$timestamp-$$"
build_dir="$OSLL_RESULT_DIR/ll"
context_tmp="$OSLL_RESULT_DIR/tmp"

mkdir -p "$build_dir" "$context_tmp" || exit 1
print "Compiling RWLOCK_COVERS_LOCKS fixture"
(
	cd "$build_dir" || exit 1
...
OSLL_LL_FILE="$build_dir/rwlock-covers-locks.ll"
...
case "$analyze_status" in
0)
	print "OSLL RWLOCK_COVERS_LOCKS analysis completed without findings."
	;;
5)
	print "OSLL RWLOCK_COVERS_LOCKS analysis completed with findings."
	;;
*)
	print "OSLL RWLOCK_COVERS_LOCKS analysis returned status $analyze_status."
	;;
esac
