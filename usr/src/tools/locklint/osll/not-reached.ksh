...
#
# Run focused NOT_REACHED path-termination cases under Solaris LockLint.
#
...
run_session()
{
	...
	for root in \
	    access_after_not_reached \
	    terminated_locked_branch \
	    only_locked_path_reaches_access \
	    balanced_live_path
	do
		run_command "declare-root-$root" "$LOCK_LINT" declare root \
		    "not-reached.c:$root" || exit $?
	done
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/not-reached.c"
...
OSLL_LL_FILE="$build_dir/not-reached.ll"
...
print "Running OSLL NOT_REACHED fixture"
...
case "$analyze_status" in
0)
	print "OSLL NOT_REACHED analysis completed without findings."
	;;
5)
	print "OSLL NOT_REACHED analysis completed with findings."
	;;
*)
	print "OSLL NOT_REACHED analysis returned status $analyze_status."
	;;
esac
