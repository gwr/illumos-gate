...
#
# Run focused rw_tryupgrade result-sensitive cases under Solaris LockLint.
#
...
run_session()
{
	...
	for root in \
	    tryupgrade_truth \
	    tryupgrade_negated \
	    tryupgrade_equal_zero \
	    tryupgrade_saved_result \
	    tryupgrade_ignored_result \
	    call_balanced_tryupgrade \
	    tryupgrade_order \
	    tryupgrade_unheld \
	    tryupgrade_writer
	do
		run_command "declare-root-$root" "$LOCK_LINT" declare root \
		    "rwlock-tryupgrade.c:$root" || exit $?
	done
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/rwlock-tryupgrade.c"
...
OSLL_RESULT_DIR="$RESULTS_ROOT/rwlock-tryupgrade-$timestamp-$$"
build_dir="$OSLL_RESULT_DIR/ll"
context_tmp="$OSLL_RESULT_DIR/tmp"

mkdir -p "$build_dir" "$context_tmp" || exit 1
print "Compiling rwlock-tryupgrade fixture"
(
	cd "$build_dir" || exit 1
...
OSLL_LL_FILE="$build_dir/rwlock-tryupgrade.ll"
...
print "Running OSLL rwlock-tryupgrade fixture"
"$LOCK_LINT" start "$SCRIPT" >"$OSLL_RESULT_DIR/session.out" 2>&1
session_status=$?
print -- "$session_status" >"$OSLL_RESULT_DIR/session.status"
if [[ "$session_status" -ne 0 ]]; then
	print -u2 "OSLL rwlock-tryupgrade run failed with status $session_status"
...
case "$analyze_status" in
0)
	print "OSLL rwlock-tryupgrade analysis completed without findings."
	;;
5)
	print "OSLL rwlock-tryupgrade analysis completed with findings."
	;;
*)
	print "OSLL rwlock-tryupgrade analysis returned status $analyze_status."
	;;
esac
