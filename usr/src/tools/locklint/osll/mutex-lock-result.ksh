...
#
# Run focused user mutex_lock result-sensitive cases under Solaris LockLint.
#
...
run_session()
{
	...
	for root in \
	    ...
	    lock_saved_result \
	    lock_ignored_result \
	    call_balanced_mutex_lock \
	    lock_order_on_success \
	    lock_already_held
	do
		run_command "declare-root-$root" "$LOCK_LINT" declare root \
		    "mutex-lock-result.c:$root" || exit $?
	done
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/mutex-lock-result.c"
...
OSLL_RESULT_DIR="$RESULTS_ROOT/mutex-lock-result-$timestamp-$$"
build_dir="$OSLL_RESULT_DIR/ll"
context_tmp="$OSLL_RESULT_DIR/tmp"

mkdir -p "$build_dir" "$context_tmp" || exit 1
print "Compiling mutex-lock-result fixture"
(
	cd "$build_dir" || exit 1
	"$CC" -Zll "$SOURCE"
...
OSLL_LL_FILE="$build_dir/mutex-lock-result.ll"
...
print "Running OSLL mutex-lock-result fixture"
"$LOCK_LINT" start "$SCRIPT" >"$OSLL_RESULT_DIR/session.out" 2>&1
session_status=$?
print -- "$session_status" >"$OSLL_RESULT_DIR/session.status"
if [[ "$session_status" -ne 0 ]]; then
	print -u2 "OSLL mutex-lock-result run failed with status $session_status"
...
case "$analyze_status" in
0)
	print "OSLL mutex-lock-result analysis completed without findings."
	;;
5)
	print "OSLL mutex-lock-result analysis completed with findings."
	;;
*)
	print "OSLL mutex-lock-result analysis returned status $analyze_status."
	;;
esac
