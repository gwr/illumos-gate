...
#
# Run focused rw_downgrade ownership and access cases under Solaris LockLint.
#
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/rwlock-downgrade.c"
...
OSLL_RESULT_DIR="$RESULTS_ROOT/rwlock-downgrade-$timestamp-$$"
build_dir="$OSLL_RESULT_DIR/ll"
context_tmp="$OSLL_RESULT_DIR/tmp"

mkdir -p "$build_dir" "$context_tmp" || exit 1
print "Compiling rwlock-downgrade fixture"
(
	cd "$build_dir" || exit 1
...
OSLL_LL_FILE="$build_dir/rwlock-downgrade.ll"
...
print "Running OSLL rwlock-downgrade fixture"
"$LOCK_LINT" start "$SCRIPT" >"$OSLL_RESULT_DIR/session.out" 2>&1
session_status=$?
print -- "$session_status" >"$OSLL_RESULT_DIR/session.status"
if [[ "$session_status" -ne 0 ]]; then
	print -u2 "OSLL rwlock-downgrade run failed with status $session_status"
...
case "$analyze_status" in
0)
	print "OSLL rwlock-downgrade analysis completed without findings."
	;;
5)
	print "OSLL rwlock-downgrade analysis completed with findings."
	;;
*)
	print "OSLL rwlock-downgrade analysis returned status $analyze_status."
	;;
esac
