...
#
# Run the focused asserted-requirement alias cases under Solaris LockLint.
#
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/assertion-alias.c"
...
run_root="$RESULTS_ROOT/assertion-alias-$timestamp-$$"
...
run_variant()
{
	...
	print "Compiling assertion-alias variant $variant"
	(
		cd "$build_dir" || exit 1
		"$CC" -Zll -DASSERTION_ALIAS_VARIANT="$variant" "$SOURCE"
	...
	OSLL_LL_FILE="$build_dir/assertion-alias.ll"
	...
	print "Running OSLL assertion-alias variant $variant"
...
run_variant 1 "same_acquire_satisfies_requirement \
same_compatible_requirements" || exit $?
run_variant 2 "distinct_acquire_does_not_satisfy_requirement \
