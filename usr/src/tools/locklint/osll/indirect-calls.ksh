...
run_session()
{
	...
	for root in locked_indirect unlocked_indirect
	do
		run_command "declare-root-$root" "$LOCK_LINT" declare root \
		    "indirect-calls.c:$root" || exit $?
	done

	run_command files "$LOCK_LINT" files
	run_command funcs "$LOCK_LINT" funcs
	run_command funcs-roots-before "$LOCK_LINT" funcs -o
	run_command funcptrs-before "$LOCK_LINT" funcptrs
	run_command pointer-calls "$LOCK_LINT" pointer calls
	run_command declare-target "$LOCK_LINT" declare \
	    "indirect-calls.c:indirect_ops.read" targets \
	    "indirect-calls.c:read_value" || exit $?
	run_command funcptrs-after "$LOCK_LINT" funcptrs
	run_command funcs-roots-after-target "$LOCK_LINT" funcs -o
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/indirect-calls.c"
...
OSLL_RESULT_DIR="$RESULTS_ROOT/indirect-calls-$timestamp-$$"
...
OSLL_LL_FILE="$build_dir/indirect-calls.ll"
