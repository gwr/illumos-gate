...
run_session()
{
	...
	if [[ "$OSLL_VARIANT" = acquisitions ]]; then
		for root in \
		    order_valid \
		    order_direct_inversion \
		    order_transitive_inversion \
		    ...
		    order_wrapped_release_before \
		    order_wrapped_acquire_before \
		    mixed_order_valid \
		    mixed_order_inversion
		do
			run_command "declare-root-$root" "$LOCK_LINT" declare root \
			    "lock-order.c:$root" || exit $?
		done
	fi
	...
	run_command order-before "$LOCK_LINT" order
	...
	0|5)
		...
		run_command order-after "$LOCK_LINT" order
		exit 0
		;;
	*)
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/lock-order.c"
CYCLE_SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/lock-order-cycle.c"
...
run_root="$RESULTS_ROOT/lock-order-$timestamp-$$"
...
run_variant()
{
	...
	print "Compiling $variant lock-order fixture"
	(
		cd "$build_dir" || exit 1
		"$CC" -Zll "$source"
	...
	OSLL_LL_FILE="$build_dir/$(basename "$source" .c).ll"
	if [[ ! -f "$OSLL_LL_FILE" ]]; then
		ls -la "$build_dir" >>"$OSLL_RESULT_DIR/compile.out" 2>&1
		print -u2 "cc -Zll did not create $variant database"
		print -u2 "see $OSLL_RESULT_DIR/compile.out"
		return 1
	fi

	OSLL_SESSION_MODE=1
	OSLL_VARIANT="$variant"
	TMPDIR="$context_tmp"
	export OSLL_RESULT_DIR OSLL_LL_FILE OSLL_SESSION_MODE OSLL_VARIANT TMPDIR

	print "Running OSLL for $variant lock order"
...
run_variant acquisitions "$SOURCE" || exit $?
run_variant declaration-cycle "$CYCLE_SOURCE" || exit $?
