...
#
# Run focused user mutex_trylock result-sensitive cases under Solaris
# LockLint.
#
...
run_session()
{
	...
	for root in \
	    trylock_equal_zero \
	    trylock_not_equal_zero \
	    trylock_truth \
	    trylock_negated \
	    trylock_saved_result \
	    trylock_ignored_result \
	    call_balanced_trylock \
	    trylock_order_on_success \
	    trylock_already_held
	do
		run_command "declare-root-$root" "$LOCK_LINT" declare root \
		    "mutex-trylock.c:$root" || exit $?
	done
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/mutex-trylock.c"
...
OSLL_LL_FILE="$build_dir/mutex-trylock.ll"
...
case "$analyze_status" in
0)
	print "OSLL mutex-trylock analysis completed without findings."
	;;
5)
	print "OSLL mutex-trylock analysis completed with findings."
	;;
*)
	print "OSLL mutex-trylock analysis returned status $analyze_status."
	;;
esac
