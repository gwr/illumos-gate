...
run_session()
{
	...
	for root in \
	    ...
	    array_same_symbolic \
	    array_different_symbolic \
	    container_alias_same \
	    container_alias_different
	do
		run_command "declare-root-$root" "$LOCK_LINT" declare root \
		    "identity-aliases.c:$root" || exit $?
	done
...
SOURCE="$REPO_ROOT/usr/src/tools/locklint/tests/identity-aliases.c"
...
OSLL_RESULT_DIR="$RESULTS_ROOT/identity-aliases-$timestamp-$$"
...
OSLL_LL_FILE="$build_dir/identity-aliases.ll"
