 ...
 * Warlock pseudo-kernel provided.  An annotation probe lets the comparison
 * verify that OSLL retained source annotations.
 */
...
struct locklint_usb_ah_annotation_probe {
	kmutex_t lock;
	int value;
};

_NOTE(MUTEX_PROTECTS_DATA(locklint_usb_ah_annotation_probe::lock,
    locklint_usb_ah_annotation_probe::value))
