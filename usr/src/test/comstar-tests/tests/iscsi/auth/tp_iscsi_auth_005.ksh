...
#	with bi-directional "CHAP" authentication by initiator and
#	be verified by iSCSI initiator
#
# STRATEGY:
#	Setup:
...
#		    chap user2 and secret2 on target host by itadm modify-target
#		    option
#		Modify the specified target node authentication property on
#		    initiator host by the same chap user2 and secret2 on target
...
# __stc_assertion_end
#
function iscsi_auth_005
{
	cti_pass

        tc_id="iscsi_auth_005"

	tc_desc="iSCSI target port provider can support connection and LU" 
	tc_desc="${tc_desc} discovery with bi-directional CHAP authentication"
	tc_desc="${tc_desc} by initiator and be verified by iSCSI initiator"
