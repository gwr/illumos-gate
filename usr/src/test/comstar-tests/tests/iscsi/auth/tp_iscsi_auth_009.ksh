...
#	"CHAP" authentication by initiator configured when mis-matched chap
#	user and secret authentication fails and verify its failure
#	on initiator host
#
# STRATEGY:
#	Setup:
...
#		    "none" authentication
#		Modify target node authentication property with the specified
#		    chap user and secret on initiator host by 
#		    iscsiadm modify target-param option
#		Modify the specified target node authentication property on
#		    target host with the wrong chap user and secret with 
#		    configured on initiator host by itadm modify-target
...
# __stc_assertion_end
#
function iscsi_auth_009
{
	cti_pass

        tc_id="iscsi_auth_009"

	tc_desc="Attempt to establish TCP connection and discovery the LU with"
	tc_desc="${tc_desc} CHAP authentication by initiator configured when"
	tc_desc="${tc_desc} mis-matched chap user and secret authentication fails"
	tc_desc="${tc_desc} and verify its failure on initiator host"
	...
	itadm_create POS target -n ${t} -t 1 -a none
	...
	itadm_modify POS target -u "t_chap_user_wrong" "${t}" 
