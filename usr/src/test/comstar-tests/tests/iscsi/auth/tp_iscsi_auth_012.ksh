...
#
# DESCRIPTION:
#	Verify that connection fails to be created if the chap secrets of 
#	both initiator and target authentication are identical within
...
#		    chap user1 and secret1 on target host by itadm modify-target
#		    option
#		Modify the specified target node authentication property on
#		    initiator host by the same chap user1 and secret1 on target
...
#		Check that iscsi initiator node has no connection
#	Cleanup:
...
# __stc_assertion_end
#
function iscsi_auth_012
{
	cti_pass

        tc_id="iscsi_auth_012"

	tc_desc="Verify that connection fails to be created if the chap secrets"
	tc_desc="${tc_desc} of both initiator and target authentication are"
	tc_desc="${tc_desc} identical within bi-directional authentication"
	...
	itadm_modify POS target -s "123456789012" "${t}" 
	itadm_modify POS target -u "t_chap_user" "${t}" 

	# Set target chap properities on initiator host
	iscsiadm_modify POS "${ISCSI_IHOST}" target-param -C "123456789012" ${t}
