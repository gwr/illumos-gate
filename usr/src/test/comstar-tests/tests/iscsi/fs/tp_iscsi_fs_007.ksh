...
#
# DESCRIPTION:
#	Each protal is belonged to an individual target portal group,
#	Running I/O data validation on initiator host
#
# STRATEGY:
#	Setup:
#		MPXIO is enable on initiator host
#		zvol LUs (specified by VOL_MAX variable in configuration) are
...
#		Create one target node with all the portal groups supported
...
# __stc_assertion_end
#
function iscsi_fs_007
{
	cti_pass

        tc_id="iscsi_fs_007"
	tc_desc="verify I/O data validation on one portal of one target portal group"
	...
	[[ $ret -eq 0 ]] && cti_pass "tp_iscsi_fs_007: PASS"
}
