...
#
# ID: iscsi_visible_010
#
# DESCRIPTION:
#	initiator can get lun information through the target when
#	--headerdigest option of target-param is set to CRC32 for
#	a target
#
# STRATEGY:
#	Setup:
...
#		
#	Test:
#		1. Set --headerdigest option of targer-param to CRC32 for one 
#		   target.
#
#		2. Enable "SentTarget" discovery method on initiator using
#		   iscsiadm modify discovery -t enable
#
#		3. Check that the initiator can see the luns through the
#		   two targets.  
#
#	Cleanup:
...
# __stc_assertion_end
#
function tp_iscsi_visible_010
{
	cti_pass

        tc_id="tp_iscsi_visible_010"

	tc_desc="initiator can get lun information through the target when"
	tc_desc="${tc_desc} --headerdigest option of target-param is set to"
	tc_desc="${tc_desc} CRC32" 
