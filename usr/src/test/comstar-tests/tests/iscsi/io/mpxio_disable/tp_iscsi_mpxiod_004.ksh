...
#
# DESCRIPTION:
#	Creation/deletion of iscsi target node by itadm on the target side 
...
#		delete one target node from target host
#		    by itadm delete-target option
#		sleep 24 seconds
#		create the same target node with the previous property 
#		    from target host by itadm create-target option
#		sleep 24 seconds
#		iterate all the target nodes then create/delete them from 
#		    target host by itadm create/delete-target option
#		repeat the create/delete operation for the specified rounds
...
# __stc_assertion_end
#
function iscsi_mpxiod_004
{
	cti_pass
	tc_id="iscsi_mpxiod_004"
	tc_desc="I/O data validation with mpxio disabled by delete/create"
	tc_desc="$tc_desc target node on target host"
	...
	iscsi_target_port_delete_create_io $ISCSI_THOST $TS_SNOOZE $TS_MAX_ITER
