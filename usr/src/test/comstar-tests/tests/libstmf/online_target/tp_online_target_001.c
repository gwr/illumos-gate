...
stmfOnlineTarget001()
{
	...
	stmfRet = stmfOnlineTarget(&(targetList->devid[0]));
	if (stmfRet != STMF_STATUS_SUCCESS) {
		ret = 6;
		goto cleanup;
	}

	while (targetProps.status != STMF_TARGET_PORT_ONLINE &&
	    retries++ < 600) {
		stmfRet = stmfGetTargetProperties(&(targetList->devid[0]),
		    &targetProps);
		if (stmfRet != STMF_STATUS_SUCCESS) {
		...
		if (targetProps.status != STMF_TARGET_PORT_ONLINE) {
			(void) nanosleep(&rqtp, NULL);
		}
	}
	if (targetProps.status != STMF_TARGET_PORT_ONLINE) {
