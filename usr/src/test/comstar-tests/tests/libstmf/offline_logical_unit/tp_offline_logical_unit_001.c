...
stmfOfflineLogicalUnit001()
{
	...
	(void) system("touch /tmp/stmfOfflineLogicalUnit.lu");
	(void) system("sbdadm create-lu -s 10g"
	    " /tmp/stmfOfflineLogicalUnit.lu");
	...
	if (luProps.status != STMF_LOGICAL_UNIT_OFFLINE) {
		ret = 6;
		goto cleanup1;
	}
...
cleanup:
	...
	(void) system("rm /tmp/stmfOfflineLogicalUnit.lu");
	return (ret);
}
...
main()
{
	return (stmfOfflineLogicalUnit001());
}
