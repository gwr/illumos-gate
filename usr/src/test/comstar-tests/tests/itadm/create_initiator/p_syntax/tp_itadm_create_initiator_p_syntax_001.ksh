#
# CDDL HEADER START
#
# The contents of this file are subject to the terms of the
# Common Development and Distribution License (the "License").
# You may not use this file except in compliance with the License.
#
# You can obtain a copy of the license at usr/src/OPENSOLARIS.LICENSE
# or http://www.opensolaris.org/os/licensing.
# See the License for the specific language governing permissions
# and limitations under the License.
#
# When distributing Covered Code, include this CDDL HEADER in each
# file and include the License file at usr/src/OPENSOLARIS.LICENSE.
# If applicable, add the following below this CDDL HEADER, with the
# fields enclosed by brackets "[]" replaced with your own identifying
# information: Portions Copyright [yyyy] [name of copyright owner]
#
# CDDL HEADER END
#

#
# Copyright 2009 Sun Microsystems, Inc.  All rights reserved.
# Use is subject to license terms.
#

#
# A test purpose file to test functionality of the create-initiator
# subfunction of the itadm command
#

# __stc_assertion_start
#
# ID: itadm_create_initiator_p_syntax_001
#
# DESCRIPTION:
# 	itadm create-initiator command successfully creates the specified
# 	initiator with all valid combinations of options
#
# STRATEGY:
# 	Create a dynamic test case that generates command and options
# 	combinations with a scope of the valid arguments
#
#	Setup:
#
#	Test:
#	 	1. itadm create-initiator
#
#		   Valid arguments
#	 		-u --chap-user
#				test-user-name
#				A String of 255 characters
#	 		-s --chap-secret
#				valid-secret
#				a-16-characters-
#	 		-S --chap-secret-path
#				valid-secret     # secret contained in file
#				a-16-characters- # secret contained in file
#			node-name
#				223-characters
#
#	 	2. Verify that the correct initiator was created using the itadm
#	 	   list-initiator command
#	Cleanup:
#		Delete the created initiator
#
# 	STRATEGY_NOTES:
#
#
# TESTABILITY: explicit
#
# AUTHOR: bobbie.long@sun.com zheng.he@sun.com
#
# REVIEWERS:
#
# ASSERTION_SOURCE:
# 	http://www.opensolaris.org/os/project/iser/itadm_1m_v4.pdf
#
# TEST_AUTOMATION_LEVEL: automated
#
# STATUS: IN_PROGRESS
#
# COMMENTS:
#
# __stc_assertion_end
#

CMD="itadm_create POS initiator ^SET_0 ${IQN_INITIATOR} | \
	itadm_create POS initiator ^STR_223 " 

SET_0=" \
	^SET_1 | \
	^SET_2 | \
	^SET_3 | \
"

SET_1=" \
	-u test-user-name | \
	-u ^STR_255 | \
"

SET_2=" \
	-s valid-secret | \
	-s a-16-characters- | \
"

SET_3=" \
	-S valid-secret | \
	-S a-16-characters- | \
"

STR_255="\
iqn.1986-03.com.sun:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\
Total-255-characters-aaaaaaaaaaaaaaaaaaaaaaaaaaaaa\
aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\
aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\
aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\
aaaaa
"

STR_223="\
iqn.1986-03.com.sun:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\
Total-223-characters-aaaaaaaaaaaaaaaaaaaaaaaaaaaaa\
aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\
aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\
aaaaaaaaaaaaaaaaaaaaaaa"

set -A CMD_ARR

function itadm_create_initiator_p_syntax
{
	cti_pass
	typeset -i i
	(( i = ${TET_TPNUMBER} - 1 ))	

        tc_id="itadm_create_initiator_p_syntax_${TET_TPNUMBER}"
	tc_desc="itadm create-initiator command successfully create initiator"
	tc_desc="${tc_desc} with { "
	tc_desc="${tc_desc} ${CMD_ARR[${i}]} }"

	print_test_case $tc_id - $tc_desc
	${CMD_ARR[${i}]}

	tp_cleanup

}

auto_generate_tp "CMD" "CMD_ARR" "itadm_create_initiator_p_syntax"

