...
cti_execute_cmd "cp_mmap -t c -f ${testdir}/${test_file} ${test_file}"
if (($?!=0)); then
	cti_fail "FAIL: cp_mmap -t c -f ${testdir}/${test_file} ${test_file} failed"
	return
else
	cti_report "PASS: cp_mmap -t c -f ${testdir}/${test_file} ${test_file} succeeded"
fi
