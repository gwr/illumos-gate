...
cti_execute_cmd "cp_mmap -t d -f ${test_file} ${testdir}/${test_file}"
if (($?!=0)); then
	cti_fail "FAIL: cp_mmap -t d -f ${test_file} ${testdir}/${test_file} failed"
	return
else
	cti_report "PASS: cp_mmap -t d -f ${test_file} ${testdir}/${test_file} succeeded"
fi
