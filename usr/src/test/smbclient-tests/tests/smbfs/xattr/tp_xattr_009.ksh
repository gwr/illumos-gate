...
typeset -i j=100
while  [ $i -lt $j ]; do
  create_xattr test_file passwd$i /etc/passwd
  i=$i+1
done
...
cti_execute_cmd "runat test_file ls passwd* > output.$$"
...
cti_execute_cmd "runat test_file rm -f passwd*"
cti_execute PASS "runat test_file ls passwd*"
if [[ $? == 0 ]]; then
