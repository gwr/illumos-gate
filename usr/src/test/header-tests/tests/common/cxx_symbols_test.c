...
/*
 * This program tests C++ symbol visibility in different compilation
 * environments.
 */
...
static char *compilation = "cxx-symbols-env.cfg";
...
mkworkdir(void)
{
	...
	(void) strlcpy(b, "/tmp/symcxx_testXXXXXX", sizeof (b));
	if ((d = mkdtemp(b)) == NULL) {
		perror("mkdtemp");
		return (-1);
	}
	dname = mystrdup(d);
	myasprintf(&cfile, "%s/compile_test.cc", d);
...
find_cxx_includes(void)
{
	...
	/*
	 * buf now contains e.g.:
	 *   /opt/gcc-14/lib/gcc/x86_64-pc-solaris2.11/14.2.0/include
	 *
	 * Find "/lib/gcc/" to split:
	 *   prefix = "/opt/gcc-14"
	 *   rest   = "x86_64-pc-solaris2.11/14.2.0/include"
	 */
	 ...
	 *   target  is the compiler target, e.g. "x86_64-pc-solaris2.11"
	 *   version is the compiler version, e.g. "14.2.0"
	 *   include is the literal directory name "include"
	 */
	rest = p + strlen("/lib/gcc/");

	/* target: up to first slash, e.g. "x86_64-pc-solaris2.11" */
	slash = strchr(rest, '/');
	if (slash == NULL) {
...
/*
 * Main entry point for the C++ symbol visibility test program.
 */
int
