...
- C++ include path setup (`find_cxx_includes()`): queries the compiler
  ...
  details.
- Brace-initialisation in `func` probe programs (see below).
...
In the C++ program, the return value uses brace-initialisation instead:
```cpp
double
test_func(double a0)
{
...
```
This is intentional: brace-initialisation prohibits narrowing conversions,
so a missing `float` or `long double` overload that would silently promote
through `double` is caught as a compile error rather than a silent pass.
