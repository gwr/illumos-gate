...
class TestDriver:
    ...
    def run(self, entries, env_config, compiler, mflag, arch,
        ...
        # drain() prints any run of already-finished jobs starting at
        # next_to_print, so output appears in job (array) order even
        # though jobs complete in whatever order the threads finish.
        ...
        # All jobs have finished or been skipped.  Print anything still
        # buffered, in order, stepping over any that did not run.
        with lock:
