...
def parse_cfg(fileobj, filename, handlers):
    ...
    Unknown keywords cause a hard error via sys.exit().
    """
    lineno = 0
    accum = ''
    for raw in fileobj:
...
class EnvConfig:
    ...
    def load(self, path):
        ...
        # Search order mirrors the C program:
        #   1. path as given
    ...
    def _parse(self, fileobj, filename='<input>'):
        ...
        validated against this value; a mismatch is a hard error.
        """
        handlers = {
...
class ProbeGen:
    ...
    def gen_probe(entry, lang):
        ...
        Output is byte-for-byte compatible with c_symbols_test / cxx_symbols_test.
        """
        out = []
    ...
    def _split_rtype(rtype):
        """
        Split rtype for function pointer support, replicating the C rtype loop.
        ...
        for i, ch in enumerate(rtype):
            if ch == '(':
                prefix = rtype[:i + 1] + rtype[i + 1]  # up to and including '(*'
                suffix = rtype[i + 2:]                   # from ')' onward
...
class TestDriver:
    ...
    def run(self, entries, env_config, compiler, mflag, arch,
        ...
        Returns True if all tests passed (and not interrupted), False otherwise.
        """
        lock = threading.Lock()
...
def _validate_c_compiler(cc, mflag, tmpdir):
    """Return True if cc is a usable C compiler (gcc or clang); False otherwise."""
...
def _validate_cxx_compiler(cc, mflag, tmpdir):
    """Return True if cc is a usable C++ compiler (g++ or clang++); False otherwise."""
