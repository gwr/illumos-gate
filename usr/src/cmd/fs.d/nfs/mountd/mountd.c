 ...
 * (when mount_port is specified) in which case we'll use the
...
md_svc_tp_create(struct netconfig *nconf)
{
	...
	/*
	 * If mount_port is set and this is an inet transport,
...
main(int argc, char *argv[])
{
	...
	ret = nfs_smf_get_iprop("mountd_port", &mount_port,
	    DEFAULT_INSTANCE, SCF_TYPE_INTEGER, NFSD);
	if (ret != SA_OK) {
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	if (mount_port < 0 || mount_port > UINT16_MAX) {
