...
smbd_spnego_accept_sec_ctx(void **auth_ctx, ndr_xa_t *mxa)
{
	...
	*out_size = 0;
	...
	if (major == GSS_S_COMPLETE) {
		OM_uint32 test_flags = 0;

		switch (auth_level) {
		case NDR_C_AUTHN_LEVEL_DEFAULT:
		case NDR_C_AUTHN_LEVEL_CONNECT:
			break;

		case NDR_C_AUTHN_LEVEL_PKT_PRIVACY:
			test_flags |= GSS_C_CONF_FLAG;
			/* FALLTHRU */
		case NDR_C_AUTHN_LEVEL_PKT_INTEGRITY:
			test_flags |= GSS_C_INTEG_FLAG|GSS_C_SEQUENCE_FLAG;
			/* FALLTHRU */
		case NDR_C_AUTHN_LEVEL_PKT:
		case NDR_C_AUTHN_LEVEL_CALL:
			test_flags |= GSS_C_REPLAY_FLAG;

			if ((test_flags & ret_flags) == test_flags)
				break;
			/* FALLTHRU */
		default:
