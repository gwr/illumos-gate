...
try_general(void)
{
	...
	printf("smb_lookup_lname(test) ret %d\n", rc);

	/* Test something that DOES require privilege. */
	rc = smb_notify_dc_changed();
	if (rc == 0) {
		printf("smb_notify_dc_changed() allowed!\n", rc);
	} else {
...
try_shares(void)
{
	...
	if (x == NERR_InternalError) {
		...
		printf("smb_share_delete() was allowed!\n");
	} else {
		printf("smb_share_delete() ret %d (?)\n", x);
	}
}
