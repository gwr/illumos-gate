...
ST_PRODUCT_NAME="NCP3 Containers"
ST_PRODUCT_REV="1.0"
ST_PRODUCT_UUID="urn:uuid:113bc025-1dd2-11b2-8071-00144f4ab4bf"
...
f_sanity_vers=$(gettext  "The image release version must be 11 (got %s), the zone is not usable on this system.")
f_not_ncp3_image=$(gettext  "%s doesn't look like an NCP3 image.")
f_sanity_nopatch=$(gettext "Unable to determine the image's patch level.")
f_sanity_downrev=$(gettext "The image patch level is downrev for running in an ncp3 branded zone.\n(patchlist %s)")
f_need_newer_emul=$(gettext "The image requires a newer version of the ncp3 brand emulation.")
...
sanity_ok=$(gettext     "  Sanity Check: Passed.  Looks like an NCP3 image.")
...
sanity_check()
{
	...
	#
	# Check image release to be sure its NCP3.
	#
	image_vers="unknown"
	if [[ -f $dir/etc/lsb-release ]]; then
		image_vers=$(nawk -F= '{
	if ($1 == "DISTRIB_DESCRIPTION") print $2}' \
		    $dir/etc/lsb-release)
	fi
	...
	*)
	...
	#
	# Check the core kernel pkg for the required KU patch. XXX
	#

	#
	# Check the NCP3 image for a required version of the emulation.
	# (removed, for now)
	#
