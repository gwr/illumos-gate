...
sanity_ok=$(gettext     "  Sanity Check: Passed.  Looks like an NCP3 system.")
...
trap_exit()
{
	if (( $EXIT_CODE != $ZONE_SUBPROC_OK )); then
		/usr/lib/brand/ncp3/uninstall $ZONENAME $ZONEPATH -F
	fi
...
# only be called by zoneadm to install an ncp3 system image into a zone.
#
...
#
/usr/lib/brand/ncp3/p2v -l "$LOGFILE" -m "$p2v_prog" \
