...
#
# Run the ncp3_support boot hook.
#
/usr/lib/brand/ncp3/ncp3_support boot $ZONENAME
if (( $? != 0 )) ; then
        exit 1
fi

BRANDDIR=/.SUNWnative/usr/lib/brand/ncp3;
FILEDIR=$BRANDDIR/files;
EXIT_CODE=1

#
# Replace the specified file in the booting zone with a wrapper script that
# invokes ncp3_isaexec_wrapper.  This is a convenience function that reduces
...
replace_with_native() {
	...
	if [ ! -h $path_dname -a -d $path_dname ]; then
		safe_replace $ZONEROOT/$1 $BRANDDIR/ncp3_isaexec_wrapper $2 $3 \
		    remove
	fi
}
...
replace_with_native_py() {
	...
	if [ ! -h $path_dname -a -d $path_dname ]; then
		safe_replace $ZONEROOT/$1 $BRANDDIR/ncp3_python_wrapper $2 $3 \
		    remove
	fi
}

#
# Create a new wrapper script that invokes ncp3_isaexec_wrapper in the
# brand (for a non-existing NCP3 file) pointing to the native brand file.
#
# Parameters:
...
wrap_with_native() {
	...
	if [ ! -h $path_dname -a -d $path_dname -a ! -f $ZONEROOT/$1 ]; then
		safe_wrap $ZONEROOT/$1 $BRANDDIR/ncp3_isaexec_wrapper $2 $3
	fi
}
...
# BINARY REPLACEMENT
#
# This section of the boot script is responsible for replacing NCP3
...
# that binary replacement will proceed safely.  Second, NCP3 binaries
...
#
# Details about the binary replacement procedure can be found in the NCP3
...
# STEP TWO
#
# Replace NCP3 binaries with Nevada binaries.
#
...
#
# Create ipmgmtd wrapper to native binary in ncp3 container
...
# IP/TCP/UDP/SCTP/ICMP tunables. If NCP3 ndd(1M) is used within an
# NCP3 container, the kernel will return EINVAL. So we need this.
#
replace_with_native /usr/sbin/ndd 0555 root:bin
