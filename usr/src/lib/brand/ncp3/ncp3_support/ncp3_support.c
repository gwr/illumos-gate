...
/*
 * ncp3_support is a small cli utility used to perform some brand-specific
...
usage_err(void)
{
	(void) printf(gettext("%s ncp3 brand error: invalid usage\n"), bname);

	(void) fprintf(stderr,
	    gettext("usage:\t%s verify <xml_file>\n"),
	    gettext("\t%s boot <zone_name>\n\n"),
	    bname, bname);
...
err(char *msg, ...)
{
	...
	(void) printf(gettext("%s ncp3 brand error: %s\n"), bname, buf);
...
/*ARGSUSED*/
static int
do_boot(char *zonename)
{
	/* Todo: check installed NCP patch level, etc. */
	return (0);
}
