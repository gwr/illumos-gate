...
zfs_groupmember(zfsvfs_t *zfsvfs, uint64_t id, cred_t *cr)
{
	...
	if (ksid && id != IDMAP_WK_CREATOR_GROUP_GID) {
		...
		if (idx != 0) {
			...
			if (strcmp(ksid_getdomain(ksid), domain) &&
			    rid == ksid_getrid(ksid))
				return (B_TRUE);

			if (ksidlist && ksidlist_has_sid(ksidlist, domain, rid))
				return (B_TRUE);
		} else {
			if (ksid_getid(ksid) == rid)
				return (B_TRUE);

			if (ksidlist && ksidlist_has_pid(ksidlist, rid))
				return (B_TRUE);
		}
