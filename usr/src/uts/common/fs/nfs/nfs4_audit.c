 ...
 */
boolean_t
nfs_audit_init(struct svc_req *req, cred_t *cr, char **path, vnode_t *vp)
{
	t_audit_data_t *tad;
	int rc;
	...
	*path = NULL;
	...
	if (!AU_ZONE_AUDITING(NULL) || auditev(AUE_SACL, cr) == 0 ||
	    req->rq_vers < NFS_V4)
		return (B_FALSE);
    ...
    boolean_t success, caller_context_t *ct, char *path)
{
	...
	if (AU_SACL_MASK_MATCHES(tad->tad_sacl_mask, desired, success))
    ...
    char *name, boolean_t success, caller_context_t *ct, char *path)
{
	...
	if (desired == ACE_DELETE_CHILD ||
		...
		DTRACE_PROBE2(nfsaudit__mask__notset__delete,
		    uint32_t, desired,
		    uint_t, num_notset);
...
boolean_t
nfs_audit_rename_init(struct svc_req *req, cred_t *cr, char **src, vnode_t *vp,
    ...
    boolean_t success, boolean_t isdir)
{
	...
	if (num_notset != 0)
		DTRACE_PROBE2(nfsaudit__mask__notset__rename,
		    uint32_t, desired,
		    uint_t, num_notset);

	/* Rename deletes the source and the target */
	desired = ACE_DELETE;
	if (src != NULL) {
		if (AU_SACL_MASK_MATCHES(tad->tad_sacl_mask_src,
		    desired, success)) {
			audit_sacl(src, cr, desired, success);
		}
		kmem_free(src, NFS_AUDIT_PATHLEN);
	}
	if (dest != NULL) {
		if (AU_SACL_MASK_MATCHES(tad->tad_sacl_mask_dest,
		    desired, success)) {
			audit_sacl(dest, cr, desired, success);
		}
		kmem_free(dest, NFS_AUDIT_PATHLEN);
	}

	/* Rename also adds a file/directory to the destination directory */
	desired = isdir ? ACE_ADD_SUBDIRECTORY : ACE_ADD_FILE;
	if (AU_SACL_MASK_MATCHES(tad->tad_sacl_mask, desired, success))
		audit_sacl(dpath, cr, desired, success);
	tad->tad_sacl_ctrl = SACL_AUDIT_NONE;
	kmem_free(dpath, NFS_AUDIT_PATHLEN);
}
