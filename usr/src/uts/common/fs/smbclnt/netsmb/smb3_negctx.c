    ...
    uint16_t negctx_cnt)
{
	...
	for (int i = 0; i < neg_ctxs->count; i++) {
		...
		case SMB2_PREAUTH_INTEGRITY_CAPS:
			...
			/*
			 * Get salt from the server reply.
	 ...
	 * Should be zero or one encrypt context. If none, just let
	 * nsmb_crypt_init_mech return -1, and smb2_smb_negotiate
	 * will clear the encryption capability.
	 */
	if (found_encrypt_ctx == 0) {
