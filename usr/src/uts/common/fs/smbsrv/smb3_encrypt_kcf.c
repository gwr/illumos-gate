    ...
    size_t datasize)
{
	...
	ASSERT(noncesize >= SMB3_AES128_CCM_NONCE_SIZE);

	ctx->param.ccm.ulMACSize = SMB2_SIG_SIZE;
	ctx->param.ccm.ulNonceSize = SMB3_AES128_CCM_NONCE_SIZE;
    ...
    uint8_t *auth, size_t authsize)
{
	...
	ASSERT3U(noncesize, >=, SMB3_AES128_GCM_NONCE_SIZE);

	ctx->param.gcm.pIv = nonce;
	ctx->param.gcm.ulIvLen = SMB3_AES128_GCM_NONCE_SIZE;
