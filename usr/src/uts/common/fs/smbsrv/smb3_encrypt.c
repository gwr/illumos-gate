...
static int
smb3_encrypt_gen_nonce(smb_user_t *user, uint8_t *buf, size_t maxlen)
{
	smb_session_t *s = user->u_session;
	boolean_t gcm = s->smb31_enc_cipherid ==
	    SMB3_CIPHER_AES128_GCM;
	size_t len = (gcm ? SMB3_AES128_GCM_NONCE_SIZE :
    ...
    struct mbuf_chain *out_mbc)	// cleartext
{
	...
	case SMB3_CIPHER_AES128_CCM:
		smb3_crypto_init_ccm_param(&ctx,
		    sr->th_nonce, sizeof (sr->th_nonce),
    ...
    struct mbuf_chain *out_mbc)	// transform header + ciphertext
{
	 ...
	 */
	rc = smb3_encrypt_gen_nonce(u, sr->th_nonce, sizeof (sr->th_nonce));
	if (rc != 0) {
