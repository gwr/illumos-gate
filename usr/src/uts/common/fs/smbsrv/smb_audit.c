...
smb_audit_init(smb_request_t *sr)
{
	...
	if (!AU_ZONE_AUDITING(NULL) || auditev(AUE_SACL, sr->user_cr) == 0 ||
    ...
    boolean_t success, boolean_t isdir)
{
	...
	desired = ACE_DELETE;
	if (src != NULL && AU_SACL_MASK_MATCHES(tad->tad_sacl_mask_src,\
	    desired, success)) {
		audit_sacl(src, sr->user_cr, desired, success);
	}
	if (dest != NULL && AU_SACL_MASK_MATCHES(tad->tad_sacl_mask_dest,
	    desired, success)) {
		audit_sacl(dest, sr->user_cr, desired, success);
	}
