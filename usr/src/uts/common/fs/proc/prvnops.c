...
#define	PR_OBJSPAN(obj32, obj64)	\
	...
	/*
	 * Some of the ops below drop p_lock, which means if the process
	 * entered zombie state or is exiting, we need to not try those ops.
	 * Enough exist below where we check now. If checking now is too broad,
	 * we will need to put this check just before any drops of p_lock.
	 */
