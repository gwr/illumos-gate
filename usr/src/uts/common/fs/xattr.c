	...
	*vpp = NULL;
	...
	if (dvp->v_xattrdir != NULL) {
		...
		if (dvp->v_xattrdir != NULL) {
			 ...
			 * just call VN_RELE(gfs_vp), because the vnode
	...
	/*
	 * We might like to get the "real" XATTR directory here too,
	 * but if that were to fail, we'd still want to return the
	 * GFS XATTR directory created above so we can get to the
	 * system attributes.  We can delay getting the real XATTR
	 * directory until we really need it, in xattr_dir_realdir.
	 */
