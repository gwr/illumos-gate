...
#define	SIS_ROUNDUP(a, align, type) \
	(type)((((uintptr_t)(a) + align - 1) & ~align))
...
sis_init_base_struct_addr(pqi_state_t s)
{
	...
	params.mailbox[1] = (uint32_t)(intptr_t)dma_addr;
	params.mailbox[2] = (uint32_t)((uint64_t)((intptr_t)dma_addr) >> 32);
...
/*
 * []------------------------------------------------------------[]
 * | Support functions for the visable legacy fucntions		|
 * []------------------------------------------------------------[]
 */
static boolean_t
