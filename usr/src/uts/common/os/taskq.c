 ...
 * variable 'taskq_maxbuckets' limits the maximum number of buckets.
 * One extra bucket is used as an "idle bucket".
 *
 * Dispatch first tries a bucket chosen by hash, then (maybe) a few
 * following buckets (taskq_search_depth), then the idle bucket.
 * If the dispatch call allows sleep, it then attempts to extend the
 * bucket chosen by hash, and makes dispatch attempt on that bucket.
 ...
 *
 * Entries normally start life in one of the buckets (chosen by hash)
 ...
 * Dispatch first tries a bucket selected via hash, then the idle bucket.
 * If both of those fail (and depending on options) an attempt to add
 * threads to the bucket is made.
 *
...
/*
 * When a bucket has no available entries other buckets can be tried.
 * taskq_search_depth parameter limits the amount of buckets that we search
 * before failing. This is probably no longer useful now that idle threads
 * stay only briefly on their starting bucket and move to the idle bucket.
 * XXX Test with and without this, and consider removing it.
 */
int taskq_search_depth = 0;
...
static taskq_ent_t *taskq_bucket_redist(taskq_bucket_t *);
...
taskq_idlebucket_dispatch(taskq_bucket_t *b, task_func_t func, void *arg)
{
	...
	/*
	 * Need the mutex on both bucket (b) and the idle bucket (idleb).
	 * Take in that order, so the most contested is last.
	 */
	mutex_enter(&b->tqbucket_lock);
	...
	if ((idleb->tqbucket_nfree != 0) &&
		...
		/* Track the "alloc" where we moved to. */
		b->tqbucket_nalloc++;
...
taskq_dispatch(taskq_t *tq, task_func_t func, void *arg, uint_t flags)
{
	...
	if (bsize == 1) {
		 ...
		 * before we try the idle bucket. XXX Can probably remove this.
		 */
	...
	if ((flags & TQ_NOSLEEP) == 0) {
		 ...
		 */
		(void) taskq_bucket_extend(bucket);
    ...
    taskq_bucket_t *bucket, taskq_t *tq)
{
	...
	/*
	 * Special case for entry after migration to a new bucket.
...
taskq_d_thread(taskq_ent_t *tqe)
{
	...
	continue_2:
		...
		/*
		 * If no idle threads, there could be a backlog.
		 * Look for buckets with a backlog and if found,
		 * migrate to that bucket.  Search starting at
		 * the next bucket from where we were, so the
		 * search starting points will be distributed.
		 *
		 * Unlocked access is OK here.  To aovid missing a
		 * backlog due to a stale (cached) idle nfree value,
		 * check whenever the idle nfree is "small" (LT 4).
		 */
		if (idle_bucket->tqbucket_nfree < 4) {
			taskq_bucket_t	*b = bucket;
	...
	entry_freelist:
		...
		if (tqe->tqent_func == NULL) {
			...
			/*
			 * Woken for some other reason, or last thread.
			 * Still on the free list, lock held.
			 * Just wait again.
			 */
			goto entry_freelist;
		...
		/*
		 * taskq_idlebucket_dispatch will have set
		 * tqent_bucket = new_bucket, tqent_func,
		 * incremented new->tqbucket_nalloc, and
		 * taken us off the free list.
		 */
		ASSERT(tqe->tqent_func != NULL);
		ASSERT(tqe->tqent_prev == NULL);
...
taskq_destroy(taskq_t *tq)
{
	...
	if (tq->tq_buckets != NULL) {
		 ...
		 */
		mutex_enter(&tq->tq_lock);
		while (tq->tq_tcreates > tq->tq_tdeaths) {
...
taskq_bucket_overflow(void *arg)
{
	...
	tqe = taskq_bucket_extend(b);
	if (tqe == NULL) {
		tqe = taskq_bucket_redist(b);
	}
	(void) tqe;
}
 ...
 * thread to it.  Always called from a context where sleep is allowed.
 *
...
taskq_bucket_extend(taskq_bucket_t *b)
{
	 ...
	 */
	if ((tq->tq_tcreates + 1) - tq->tq_tdeaths > tq->tq_maxsize) {
		mutex_exit(&tq->tq_lock);
		return (NULL);
	}
	tq->tq_tcreates++;
 ...
 * also failed to create a new thread.  That means the taskq must have
 * as many threads are we're allowed to create.  If this bucket has a
 * backlog and no threads, then redistribute threads by moving one
 * from another bucket into this one.
 *
 * Return value is just for dtrace visibility.
 */
static taskq_ent_t *
taskq_bucket_redist(taskq_bucket_t *bucket)
{
	 ...
	 * by choosing the bucket with the most threads.
	 * Unlocked access should be OK here.
	 */
	db = bucket;
	for (bid = 0, b = tq->tq_buckets;
	     bid < tq->tq_nbuckets;
		...
		b_threads = b->tqbucket_nalloc + b->tqbucket_nfree;
		if (b_threads > nthreads) {
			nthreads = b_threads;
			db = b;
		}			
	}
	if (db == bucket || nthreads <= 1) {
		DTRACE_PROBE2(taskq__redist__fails, taskq_t *, tq,
		      taskq_bucket_t *, db);
		goto failed;
	}

	if (db->tqbucket_nfree != 0)
		tqe = taskq_bucket_dispatch(db, taskq_d_redirect, bucket);
	if (tqe == NULL)
		tqe = taskq_backlog_dispatch(db, taskq_d_redirect, bucket,
		    TQ_FRONT);
	if (tqe == NULL) {
		/* Possible during close or suspend, etc. */
	failed:
		mutex_enter(&bucket->tqbucket_lock);
		bucket->tqbucket_flags &= ~TQBUCKET_REDIRECT;
		mutex_exit(&bucket->tqbucket_lock);
...
out:
	DTRACE_PROBE3(taskq__bucket__redist__ret, taskq_t *, tq,
		      taskq_bucket_t *, bucket, taskq_ent_t *, tqe); // XXX
