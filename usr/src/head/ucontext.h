...
/*
 * Some history: illumos 5876 moved the <sys/regset.h> include here
 * that was previously in the (CPU-dependent) <sys/ucontext.h>,
 * and that solved some problems but didn't finish the job.
 * Now let programs that need the <sys/regset.h> defines do the
 * include on their own if they need those.
 */
